...
# Add a pinned_timestamp field to all posts
posts.update_many(
    { 'pinned_timestamp': { '$exists': False } },  # Find posts without pinned_timestamp
    { '$set': { 'pinned_timestamp': None } }  # Set pinned_timestamp to None by default
)

# For posts that are already pinned, set pinned_timestamp to the current time
posts.update_many(
    { 'pinned': True, 'pinned_timestamp': None },  # Find pinned posts without pinned_timestamp
    { '$set': { 'pinned_timestamp': datetime.now() } }  # Set pinned_timestamp to the current time
)
...
def utility_processor():
    ...
    return dict(
        ...
        unseen_count=unseen_count  # Pass unseen_count to all templates
    )
...
# Helper function to find the top-level parent of a comment
...
# Helper function to calculate the page number for a top-level comment
# def calculate_page_number(post_id, comment_timestamp, per_page=5):
#     # Count how many top-level comments were created before this one
...
def calculate_comment_page(post_id, comment_timestamp, per_page=5):
    ...
    count_before = comments.count_documents({
        'post_id': ObjectId(post_id),
        'parent_comment_id': None,  # Only count top-level comments
        'timestamp': {'$lt': comment_timestamp}  # Comments created before this one
    })
    return (count_before // per_page) + 1  # Calculate the page number



# Updated add_notification function
def add_notification(notification_type, message, target_user=None, target_post=None, target_comment=None):
    ...
    if target_comment:
        ...
        if target_comment.get('parent_comment_id'):
            ...
            top_level_parent = target_comment  # This is a top-level comment
        
        # Calculate the page number for the top-level parent
        page_number = calculate_comment_page(top_level_parent['post_id'], top_level_parent['timestamp'])
        
        # Generate the link with the correct page and comment anchor
...
def pin_post(post_id):
    ...
    # Pin the post and set the pinned_timestamp
    posts.update_one(
...
def unpin_post(post_id):
    ...
    # Unpin the post and clear the pinned_timestamp
    posts.update_one(
        {'_id': ObjectId(post_id)},
...
def add_comment(post_id):
    ...
    if parent_comment_id:
        ...
        top_level_parent = comment  # This is a top-level comment
    
    # Calculate the page number for the top-level parent
    page_number = calculate_comment_page(top_level_parent['post_id'], top_level_parent['timestamp'])
    
    # Add notification with the page number
    post = posts.find_one({'_id': ObjectId(post_id)})
    if post:
        add_notification(
            'comment',
            ...
            target_post=post,
            target_comment=comment  # Pass the comment object
        )
...
def view_topic(post_id):
    ...
    # Ensure only approved posts are visible to non-owners/moderators
    ...
    # Pagination logic (top-level comments only)
    ...
    # Calculate total number of top-level comments for pagination
    total_comments = comments.count_documents({'post_id': ObjectId(post_id), 'parent_comment_id': None})
    
    # Calculate total contribution for the post
    ...
    return render_template(
        'view_topic.html',
        post=post,
        author=author,  # Pass the author's details to the template
...
def dashboard():
    ...
    # fetching dashboard data ( post statistics, user activity )
    post_stats = {
...
def mark_all_notifications_seen():
    ...
    notifications.update_many(
        {'seen': False},  # Filter for unseen notifications
        {'$set': {'seen': True}}  # Mark them as seen
    )
