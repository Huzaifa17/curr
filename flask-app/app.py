...
# updating notifications to add a default type field
...
def send_otp_email(email, otp):
    try:
        ...
        # sending the email to esndgrid then it will send
...
def verify_otp(email):
    ...
    stored_otp_data = otp_storage.get(email)
    if stored_otp_data:
        expiration_time = stored_otp_data['expiration'].strftime('%Y-%m-%d %H:%M:%S')  # Formatting the expiration time so that it can fit in the front end
    else:
